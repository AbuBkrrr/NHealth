...
function PaymentRow({ payment, onConfirmed }: { payment: Payment; onConfirmed: () => void }) {
  ...
  return (
    <Card>
      ...
      <Text style={styles.meta}>Nurse visit fee</Text>
...
export function NursePaymentsScreen() {
  const [payments, setPayments] = useState<Payment[]>([]);

  const load = useCallback(() => {
    paymentApi.listIncoming().then((all) => setPayments(all.filter((p) => p.payableType === 'NURSE_REQUEST'))).catch(() => {});
  }, []);
  ...
  return (
    <FlatList
      ...
      keyExtractor={(p) => p.id}
      ListEmptyComponent={
        <EmptyState icon="💰" title="No payments waiting" subtitle="Payments patients make for visits will show up here for confirmation." />
      }
...
const styles = StyleSheet.create({
  ...
  amount: { fontSize: 16, fontWeight: '800', color: colors.nurse },
