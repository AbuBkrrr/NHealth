...
function PaymentRow({ payment, onConfirmed }: { payment: Payment; onConfirmed: () => void }) {
  ...
  return (
    <Card>
      ...
      <Text style={styles.meta}>Pharmacy order</Text>
...
export function PharmacyPaymentsScreen() {
  const [payments, setPayments] = useState<Payment[]>([]);

  const load = useCallback(() => {
    paymentApi.listIncoming().then((all) => setPayments(all.filter((p) => p.payableType === 'PHARMACY_ORDER'))).catch(() => {});
  }, []);
  ...
  return (
    <FlatList
      ...
      keyExtractor={(p) => p.id}
      ListEmptyComponent={
        <EmptyState icon="💰" title="No payments waiting" subtitle="Payments patients make for orders will show up here for confirmation." />
      }
...
const styles = StyleSheet.create({
  ...
  amount: { fontSize: 16, fontWeight: '800', color: colors.pharmacy },
